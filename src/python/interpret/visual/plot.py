...
def plot_performance_curve(data_dict,
    title='', xtitle='', ytitle='', auc_prefix='', baseline=False):
    ...
    auc_trace = go.Scatter(x=x_values, y=y_values,
                           mode='lines',
    ...
    data = [auc_trace]
    if baseline:
        baseline_trace = go.Scatter(x=[0, 1], y=[0, 1],
                                    mode='lines',
                                    line=dict(
                                        color='navy', width=width, dash='dash'
                                    ),
                                    showlegend=False)
...
def _plot_with_line(data_dict, main_fig, title='', xtitle='', ytitle='',
                       share_xaxis=False, line_name=''):

    secondary_fig = plot_line(
        data_dict['line'],
        title=title, xtitle=xtitle, ytitle=ytitle, name=line_name
    )
    figure = _two_plot(main_fig, secondary_fig, title=title, share_xaxis=share_xaxis)
    return figure


def plot_density(data_dict, title='', xtitle='', ytitle='', is_categorical=False, name='', color=COLORS[0]):
    ...
    if not is_categorical:
        ...
        for indx in range(len(edges) - 1):
            new_val = '{0} - {1}'.format(
                _pretty_number(edges[indx]),
                _pretty_number(edges[indx + 1]),
            )
...
def _plot_with_density(data_dict, main_fig, title='', xtitle='', ytitle='',
...
def plot_line(data_dict, title=None, xtitle="", ytitle="", name='Main', color=COLORS[0]):
    if data_dict.get('scores', None) is None:
        return None
...
def plot_bar(data_dict, title="", xtitle="", ytitle=""):
    ...
    # Add density
    if data_dict.get('density', None) is not None:
        figure = _plot_with_density(data_dict['density'], main_fig, title=title, is_categorical=True)
    else:
        figure = main_fig
...
def plot_horizontal_bar(data_dict, title="", xtitle="", ytitle="", start_zero=False):
    ...
    if data_dict.get('perf', None) is not None and title == "":
        title_items = []
        title_items.append(
            "Predicted {0:.2f}".format(data_dict['perf']['predicted'])
        )
        title_items.append(
            "Actual {0:.2f}".format(data_dict['perf']['actual'])
        )
...
def sort_take(data_dict,
              sort_fn=None, sort_target='scores',
    ...
    for key in data_dict.keys():
        if key in ['names', 'scores', 'values', 'left_names', 'right_names']:
            if reverse_results:
                data_dict[key] = [
                    data_dict[key][i] for i in reversed(sort_indexes)
                ]
            else:
