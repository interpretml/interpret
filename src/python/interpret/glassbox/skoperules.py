...
class RulesExplanation(ExplanationMixin):
    ...
    def visualize(self, key=None):
        ...
        # Handle everything else as invalid
            ...
            msg = 'Not suppported: {0}, {1}'.format(
                self.explanation_type, key
            )
            log.error(msg)
            raise Exception(msg)
...
class DecisionListClassifier(ClassifierMixin, ExplainerMixin):
    ...
    def fit(self, X, y):
        ...
        self.sk_model_ = SR(
            feature_names=self.feature_index_, **self.kwargs
        )
    ...
    def _scores(self, X):
        ...
        for k, r in enumerate(selected_rules):
            ...
            scores[matched_idx] = np.minimum(
                k, scores[matched_idx]
            )
    ...
    def _extract_rules(self, rules):
        ...
        for indx, rule_rec in enumerate(rules):
            ...
            rule_fix = re.sub(pattern,
                lambda m: self.feature_map_[m.group(1)], rule_round
            )
