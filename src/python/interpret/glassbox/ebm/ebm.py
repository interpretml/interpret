    ...
    def fit(self, X):
        ...
        for col_idx in range(X.shape[1]):
            ...
            if col_info['type'] == 'continuous':
                ...
                mapping = {
                    val: indx for indx, val in enumerate(col_info['order'])
                }
                ...
                mapping = {
                    val: indx for indx, val in enumerate(uniq_vals)
                }
        ...
        for col_idx in range(X.shape[1]):
            ...
            if col_info['type'] == 'continuous':
                ...
                vec_map = np.vectorize(
                    lambda x: mapping[x]
                    if x in mapping else self.unknown_constant
                )
...
class BaseCoreEBM(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        X_train, X_val, y_train, y_val = train_test_split(
            X, y,
            test_size=self.holdout_split, random_state=self.random_state,
            stratify=y if is_classifier(self) else None
        )
        # Define attributes
        self.attributes_ = EBMUtils.gen_attributes(
            self.col_types, self.col_n_bins
        )
        ...
        with closing(NativeEBM(
            self.attributes_, main_attr_sets,
    ...
    def _build_interactions(self, native_ebm):
        if isinstance(self.interactions, int) and self.interactions != 0:
            ...
            ranked_scores = list(sorted(
                interaction_scores, key=lambda x: x[1], reverse=True
            ))
            n_interactions = min(len(ranked_scores), self.interactions)

            inter_indices_ = \
                [x[0] for x in ranked_scores[0:n_interactions]]
    ...
    def _fit_main(self, native_ebm, main_attr_sets):
        ...
        for index, attr_set in enumerate(main_attr_sets):
            self.attribute_set_models_.append(
                native_ebm.get_best_model(index)
            )
    ...
    def staged_fit_interactions(self, X, y, inter_indices=[]):
        ...
        with closing(NativeEBM(
                self.attributes_, inter_attr_sets,
                X_train, y_train, X_val, y_val,
                num_inner_bags=self.feature_step_n_inner_bags,
                num_classification_states=self.num_classes_,
                model_type=model_type,
    ...
    def _cyclic_gradient_boost(self, native_ebm,
                               attribute_sets, name=None):
        ...
        for data_episode_index in range(self.data_n_episodes):
            if data_episode_index % 10 == 0:
                log.debug(
                    'Sweep Index for {0}: {1}'.format(
                        name, data_episode_index
                    )
                )
            ...
            if no_change_run_length >= self.early_stopping_run_length:
                log.debug(
                    'Early break {0}: {1}'.format(name, data_episode_index)
                )
                break
...
class BaseEBM(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        train_model_args_iter = ((estimators[i], X, y) for i in range(self.n_estimators))
        ...
        for index, _ in enumerate(self.attribute_sets_):
            ...
            for estimator in estimators:
                log_odds_tensors.append(
                    estimator.attribute_set_models_[index]
                )
            averaged_model = np.average(
                np.array(log_odds_tensors), axis=0
            )
        ...
        for index, attribute_set in enumerate(self.attribute_sets_):
            feature_name = EBMUtils.gen_feature_name(
                ...
                self.preprocessor_.col_names_,
            )
            feature_type = EBMUtils.gen_feature_type(
                attribute_set['attributes'],
                self.preprocessor_.col_types_,
            )
    ...
    def _select_merged_pairs(self, estimators, X, y):
        ...
        for index, estimator in enumerate(estimators):
            ...
            X_train, X_val, y_train, y_val = train_test_split(
                X, y,
                test_size=self.holdout_split, random_state=estimator.random_state,
                stratify=y if is_classifier(self) else None
            )
            ...
            pair_ranks = np.mean(
                np.array([backward_ranks, forward_ranks]), axis=0
            )
        ...
        pair_weighted_ranks = sorted(
            pair_weighted_ranks.items(), key=lambda x: x[1]
        )
    ...
    def explain_global(self, name=None):
        ...
        for attribute_set_index, attribute_set in enumerate(self.attribute_sets_):
            ...
            attribute_indexes = \
                self.attribute_sets_[attribute_set_index]['attributes']

            if len(attribute_indexes) == 1:
                bin_labels = self.preprocessor_.get_bin_labels(
                    attribute_indexes[0],
                )
