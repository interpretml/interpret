...
def multiclass_postprocess(X_binned, feature_graphs, binned_predict_proba):
    '''
    ...
    for i in range(len(feature_graphs)):
        ...
        # mean = sum(np.multiply(updated_feature_graphs[i],bincount))/len(data)
    return updated_feature_graphs # Match the structure of the feature_graphs.