...
class Marginal(ExplainerMixin):
    ...
    def __init__(self, feature_names=None, feature_types=None, max_scatter_samples=400, random_state=1,**kwargs):
    ...
    def explain_data(self, X, y, name=None):
        ...
        # Sample down
        n_samples = self.max_scatter_samples if len(y) > self.max_scatter_samples else len(y)
        np.random.seed(self.random_state)
        idx = np.random.choice(
            np.arange(len(y)), n_samples, replace=False
        )
        X_sample = X[idx, :]
        y_sample = y[idx]
        specific_dicts = []
...
class MarginalExplanation(ExplanationMixin):
    ...
    def visualize(self, key=None):
        ...
        layout = go.Layout(
            ...
            title = "Pearson Correlation: {0:.3f}".format(corr) if corr is not None else ''
        )
...
class ClassHistogramExplanation(ExplanationMixin):
    ...
    def visualize(self, key=None):
        ...
        if key is None:
            ...
            layout = go.Layout(title="Response Distribution", showlegend=False, xaxis=dict(type='category'))
        ...
        for idx, current_class in enumerate(classes):
            ...
            fig = plot_density(
                data_dict,
                color=COLORS[idx], name=str(current_class),
