      ...
      do {
         ...
         while(true) {
            ...
            if(bValidation) {
               ...
               metric = TFloat::ApproxExp<false>(metric);
               metric += 1.0;
               metric = TFloat::ApproxLog<false>(metric);
               ...
               //                const FLOAT gradient = (UNPREDICTABLE(0 == target) ? FLOAT { -1 } : FLOAT { 1 }) / (FLOAT{ 1 } + ExpForBinaryClassification(UNPREDICTABLE(0 == target) ? -sampleScore : sampleScore));
               // !!! IMPORTANT: SEE ABOVE
               ...
               denominator = TFloat::ApproxExp<false>(denominator);
               denominator += 1.0;
