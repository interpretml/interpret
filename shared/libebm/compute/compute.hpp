...
template<typename T, typename U, U multiplicator, int shiftEnd, int shift> struct MultiplierInternal final {
   GPU_DEVICE inline constexpr static T Func(const T val) {
      return (U{0} != (multiplicator & (U{1} << shift)) ? (val << shift) : T{0}) +
            MultiplierInternal<T, U, multiplicator, shiftEnd, shift + 1>::Func(val);
   }
};
...
struct MultiplierInternal<T, U, multiplicator, shiftEnd, shiftEnd> final {
   GPU_DEVICE inline constexpr static T Func(const T) { return T{0}; }
};
template<typename T, typename U, U multiplicator> GPU_DEVICE inline constexpr static T MultiplyConst(const T val) {
...
template<typename T, typename U, bool bCompileTime, U multiplicator>
GPU_DEVICE inline constexpr static typename std::enable_if<!bCompileTime, T>::type Multiply(
      const T val, const U runtimeMultiplicator) {
