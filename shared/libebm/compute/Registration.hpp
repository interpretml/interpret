...
public:
   ...
   RegistrationPack(const AccelerationFlags zones, const char * sRegistrationName, const Args &... args) : Registration(zones, sRegistrationName) {
      ...
      m_callBack = [args...](
         const AccelerationFlags zonesLambda,
         const Config * const pConfig,
         const char * const sRegistration,
         const char * const sRegistrationEnd,
         void * const pWrapperOut
...
typename std::enable_if<AccelerationFlags_NONE == TFloat::k_zone || 0 != (TFloat::k_zone & zones), std::shared_ptr<const Registration>>::type Register(const char * const sRegistrationName, const Args &... args) {
...
template<typename TFloat, template <typename> class TRegistrable, AccelerationFlags zones, typename... Args>
typename std::enable_if<AccelerationFlags_NONE != TFloat::k_zone && 0 == (TFloat::k_zone & zones), std::shared_ptr<const Registration>>::type Register(const char * const, const Args &...) {
   return nullptr;
}
