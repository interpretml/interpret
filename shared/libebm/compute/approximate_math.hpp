...
// TODO: do the massive multi-dataset addExpSchraudolphTerm/addLogSchraudolphTerm  term optimization using our
// benchmarks
...
// same relative error as [-ln(2) / 2, +ln(2) / 2), [10, 10 + ln(2)) or any other shift.  We can use this property when
// testing to optimize for various outcomes, BUT our inputs needs to be evenly distributed, so we can't use
// std::nextafter. We need to increment in identical increments like "ln(2) / some_prime_number".  The period [-ln(2) /
// 2, +ln(2) / 2) is interesting because it concentrates in the region with the most reslution for floating point values
// and we could get a proven symmetric look at the data, and we could iterate with an integer which would ensure we have
// precise negative and positive inputs
...
static constexpr int32_t k_expTermZeroMeanRelativeError =
      1064870596; // experimentally determined.  This unbiases our average such that averages of large numbers of values
                  // should be balanced towards zero
static constexpr int32_t k_expTermMinimizeRelativeRMSE = 1064866805; // theoretically justified -> 1065353216 - 486411
static constexpr int32_t k_expTermMinimizeRelativeMeanAbsError =
      1064807269; // theoretically justified -> 1065353216 - 545947
static constexpr int32_t k_expTermLowerBound = 1064631197; // theoretically justified -> 1065353216 - 722019

// experimentally determined softmax Schraudolph terms for softmax where one logit is zeroed by subtracing it from the
// other logits
static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxThreeClassesZeroingSkewMinus0_696462 =
      1064873067; // experimentally determined, +-1, from -0.696462 - k_expErrorPeriodicity / 2 to -0.696462 +
                  // k_expErrorPeriodicity / 2
// mid-point = -0.509733
static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxThreeClassesZeroingSkewMinus0_323004 =
      1064873067; // experimentally determined, +-1, from -0.323004 - k_expErrorPeriodicity / 2 to -0.323004 +
                  // k_expErrorPeriodicity / 2
static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxThreeClassesZeroingSkew0 =
      1064873067; // experimentally determined, +-1, from -k_expErrorPeriodicity / 2 to +k_expErrorPeriodicity / 2
static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxThreeClassesZeroingSkewPlus0_370957 =
      1064873067; // experimentally determined, +-1, from 0.370957 - k_expErrorPeriodicity / 2 to 0.370957 +
                  // k_expErrorPeriodicity / 2
// mid-point = 0.533474
static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxThreeClassesZeroingSkewPlus0_695991 =
      1064873067; // experimentally determined, +-1, from 0.695991 - k_expErrorPeriodicity / 2 to 0.695991 +
                  // k_expErrorPeriodicity / 2

static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxThreeClassesZeroingTermZeroedRange20 =
      1064872079; // experimentally determined, +-1, the zeroed class, from -10 * k_expErrorPeriodicity / 2 to +10 *
                  // k_expErrorPeriodicity / 2
...
// k_expTermZeroMeanRelativeErrorSoftmaxThreeClassesZeroingTermNonZeroedRange20, which should be approximately correct
// IF the zeroed logit class has roughly the same number of samples as the non-zeroed classes combined.  We don't know
// the sample class makeup though (and building a table for various sample class skews would be time consuming) so
// taking the average is a pretty good guess, especially since the upper and lower values are fairly close
//
...
static constexpr int32_t k_expTermZeroMeanErrorForSoftmaxWithZeroedLogit =
      1064871915; // experimentally determined, AVERAGED between the zeroed and non-zeroed class, from -10 *
                  // k_expErrorPeriodicity / 2 to +10 * k_expErrorPeriodicity / 2
static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxThreeClassesZeroingTermNonZeroedRange20 =
      1064871750; // experimentally determined, +-1, the non-zeroed class, from -10 * k_expErrorPeriodicity / 2 to +10 *
                  // k_expErrorPeriodicity / 2
...
// static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxTwoClassesZeroingSkew0 = 1064873955; // experimentally
// determined, +-? static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxTwoClassesZeroingSkew0_370? =
// 1064873955; // experimentally determined, +-? static constexpr int32_t
// k_expTermZeroMeanRelativeErrorSoftmaxTwoClassesZeroingSkew0_69982536866359447004608294930875 = 1064873955; //
// experimentally determined, +-?

// DO NOT USE.  This constant minimizes the mean error for softmax where none of the logits are zeroed.  It has
// too much variability in the output though to be useful when compared against the softmax that zeroes one of the
// logits Using non-zeroed softmax isn't as viable since at modest skews (I tried k_expErrorPeriodicity / 4), achieving
// zero average error is impossible since all valid addition terms from k_expTermLowerBound to k_expTermUpperBound can
// result in non-zero error averages softmax with a zeroed term seems to have much tighter bounds on the mean error even
// if the absolute mean error is higher
static constexpr int32_t k_expTermZeroMeanRelativeErrorSoftmaxThreeClassesSkew0 =
      1064963329; // experimentally determined +-1, non-zeroed, from -k_expErrorPeriodicity / 2 to k_expErrorPeriodicity
                  // / 2
...
template<bool bNegateInput = false,
      ...
      typename T>
GPU_DEVICE INLINE_ALWAYS static T ExpApproxSchraudolph(
      T val, const int32_t addExpSchraudolphTerm = k_expTermZeroMeanErrorForSoftmaxWithZeroedLogit) {
   ...
   if(LIKELY(!bPassNaN)) {
      ...
      const int32_t retInt =
            static_cast<int32_t>(signedExpMultiple * valFloat + static_cast<float>(addExpSchraudolphTerm));
#endif
...
// LOG constants
static constexpr float k_logTermZeroMeanErrorForLogFrom1_To1_5 =
      -87.9865799f; // experimentally determined.  optimized for input values from 1 to 1.5.  Equivalent to 1064831465
...
template<bool bNegateOutput = false,
      ...
      bool bZeroPossible = true, // if false, positive zero returns a big negative number, negative zero returns a big
                                 // positive number
      bool bPositiveInfinityPossible = true, // if false, +inf returns a big positive number.  If val can be a double
                                             // that is above the largest representable float, then setting this is
                                             // necessary to avoid undefined behavior
      typename T>
GPU_DEVICE INLINE_ALWAYS static T LogApproxSchraudolph(
      T val, const float addLogSchraudolphTerm = k_logTermLowerBoundInputCloseToOne) {
