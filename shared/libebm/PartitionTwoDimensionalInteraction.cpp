 ...
 public:
   ...
   INLINE_RELEASE_UNTEMPLATED static double Func(InteractionCore* const pInteractionCore,
      ...
      auto* const aAuxiliaryBins =
            aAuxiliaryBinsBase->Specialize<FloatMain, UIntMain, bHessian, GetArrayScores(cCompilerScores)>();
      auto* const aBins = aBinsBase->Specialize<FloatMain, UIntMain, bHessian, GetArrayScores(cCompilerScores)>();

#ifndef NDEBUG
      auto* const aDebugCopyBins =
            aDebugCopyBinsBase->Specialize<FloatMain, UIntMain, bHessian, GetArrayScores(cCompilerScores)>();
#endif // NDEBUG
      ...
      Bin<FloatMain, UIntMain, bHessian, GetArrayScores(cCompilerScores)> bin00;
      Bin<FloatMain, UIntMain, bHessian, GetArrayScores(cCompilerScores)> bin01;
      Bin<FloatMain, UIntMain, bHessian, GetArrayScores(cCompilerScores)> bin10;
      Bin<FloatMain, UIntMain, bHessian, GetArrayScores(cCompilerScores)> bin11;
      ...
      do {
         ...
         do {
            ...
            if(LIKELY(cSamplesLeafMin <= bin00.GetCountSamples())) {
               EBM_ASSERT(2 == cRealDimensions); // our TensorTotalsSum needs to be templated as dynamic if we want to
                                                 // have something other than 2 dimensions
               TensorTotalsSum<bHessian, cCompilerScores, cCompilerDimensions>(cScores,
                     cRealDimensions,
                     aDimensions,
                     0x01,
               ...
               if(LIKELY(cSamplesLeafMin <= bin01.GetCountSamples())) {
                  EBM_ASSERT(2 == cRealDimensions); // our TensorTotalsSum needs to be templated as dynamic if we want
                                                    // to have something other than 2 dimensions
                  TensorTotalsSum<bHessian, cCompilerScores, cCompilerDimensions>(cScores,
                        cRealDimensions,
                        aDimensions,
                        0x02,
                  ...
                  if(LIKELY(cSamplesLeafMin <= bin10.GetCountSamples())) {
                     EBM_ASSERT(2 == cRealDimensions); // our TensorTotalsSum needs to be templated as dynamic if we
                                                       // want to have something other than 2 dimensions
                     TensorTotalsSum<bHessian, cCompilerScores, cCompilerDimensions>(cScores,
                           cRealDimensions,
                           aDimensions,
                           0x03,
                     ...
                     if(LIKELY(cSamplesLeafMin <= bin11.GetCountSamples())) {
                        ...
                        for(size_t iScore = 0; iScore < cScores; ++iScore) {
                           ...
                           const FloatCalc d00 = static_cast<FloatCalc>(
                                 bUseLogitBoost ? aGradientPairs00[iScore].GetHess() : bin00.GetWeight());

                           const FloatCalc n01 = static_cast<FloatCalc>(aGradientPairs01[iScore].m_sumGradients);
                           const FloatCalc d01 = static_cast<FloatCalc>(
                                 bUseLogitBoost ? aGradientPairs01[iScore].GetHess() : bin01.GetWeight());

                           const FloatCalc n10 = static_cast<FloatCalc>(aGradientPairs10[iScore].m_sumGradients);
                           const FloatCalc d10 = static_cast<FloatCalc>(
                                 bUseLogitBoost ? aGradientPairs10[iScore].GetHess() : bin10.GetWeight());
                           ...
                           if(0 != (CalcInteractionFlags_Pure & flags)) {
                              ...
                              if(FloatCalc{0} != d00 && FloatCalc{0} != d01 && FloatCalc{0} != d10 &&
                                    FloatCalc{0} != d11) {
