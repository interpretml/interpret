...
compile_file() {
   ...
   l3_compile_specific="$l3_compiler $l3_compiler_args_sanitized -DZONE_$l3_zone -c $l3_file_sanitized -o $l3_object_full_file_sanitized 2>&1"
...
compile_compute() {
   ...
   compile_directory "$l6_compiler" "$l6_compiler_args_sanitized -I$l6_src_path_sanitized/compute/${l6_zone}_ebm" "$l6_src_path_unsanitized/compute/${l6_zone}_ebm" "$l6_obj_path_unsanitized" "$l6_asm" "$l6_zone"
}
...
if [ "$os_type" = "Linux" ]; then
   ...
   if [ $release_64 -eq 1 ]; then
      ...
      specific_args="$all_args -m64 -DNDEBUG -O3 -DBRIDGE_AVX2_32 -Wl,--wrap=memcpy -Wl,--wrap=exp -Wl,--wrap=log -Wl,--wrap=log2,--wrap=pow,--wrap=expf,--wrap=logf"
      # the linker wants to have the most dependent .o/.so/.dylib files listed FIRST
      ...
      compile_directory "$cpp_compiler" "$specific_args $common_args" "$src_path_unsanitized/common_c" "$obj_path_unsanitized" "$is_asm" "C"
      compile_directory "$cpp_compiler" "$specific_args $cpu_args" "$src_path_unsanitized" "$obj_path_unsanitized" "$is_asm" "cpu"
      ...
      compile_file "$cpp_compiler" "$specific_args $cpu_args" "$src_path_unsanitized"/special/linux_wrap_functions.cpp "$obj_path_unsanitized" "$is_asm" "cpu"
   ...
   if [ $debug_64 -eq 1 ]; then
      ...
      specific_args="$all_args -m64 -O1 -DBRIDGE_AVX2_32 -Wl,--wrap=memcpy -Wl,--wrap=exp -Wl,--wrap=log -Wl,--wrap=log2,--wrap=pow,--wrap=expf,--wrap=logf"
      # the linker wants to have the most dependent .o/.so/.dylib files listed FIRST
      ...
      make_initial_paths_simple "$obj_path_unsanitized" "$bin_path_unsanitized"
      compile_directory "$cpp_compiler" "$specific_args $common_args" "$src_path_unsanitized/common_c" "$obj_path_unsanitized" 0 "C"
      compile_directory "$cpp_compiler" "$specific_args $cpu_args" "$src_path_unsanitized" "$obj_path_unsanitized" 0 "cpu"
      ...
      compile_file "$cpp_compiler" "$specific_args $cpu_args" "$src_path_unsanitized"/special/linux_wrap_functions.cpp "$obj_path_unsanitized" 0 "cpu"
      link_file "$cpp_compiler" "$link_args $specific_args" "$bin_path_unsanitized" "$bin_file"
      printf "%s\n" "$g_compile_out_full"
      printf "%s\n" "$g_compile_out_full" > "$g_log_file_unsanitized"
      copy_bin_files "$bin_path_unsanitized" "$bin_file" "$python_lib_unsanitized" "$staging_path_unsanitized"
   ...
   if [ $release_64 -eq 1 ]; then
      ...
      specific_args="$all_args -march=core2 -target x86_64-apple-macos10.12 -m64 -DNDEBUG -O3 -DBRIDGE_AVX2_32"
      # the linker wants to have the most dependent .o/.so/.dylib files listed FIRST
   ...
   if [ $debug_64 -eq 1 ]; then
      ...
      specific_args="$all_args -march=core2 -target x86_64-apple-macos10.12 -m64 -O1 -DBRIDGE_AVX2_32 -fsanitize=address,undefined -fno-sanitize-recover=address,undefined -fno-optimize-sibling-calls -fno-omit-frame-pointer"
      # the linker wants to have the most dependent .o/.so/.dylib files listed FIRST
