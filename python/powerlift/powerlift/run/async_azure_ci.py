...
async def assign_contributor_permissions(
    aci_client,
    auth_client,
    max_undead,
    credential,
    ...
    while max_undead < len(container_groups):
        ...
        try:
            ...
            container_group = await aci_client.container_groups.get(
            ...
            if resource_uris is not None:
                for resource_uri in resource_uris:
                    async def _create_role_assignment():
                        await auth_client.role_assignments.create(
                            resource_uri, str(uuid.uuid4()), ra_principal_contributor
                        )
                    tasks.append(_create_role_assignment())
...
async def run_azure_process(
    ...
    aci_client: Optional[ContainerInstanceManagementClient] = None
    auth_client = None
    container_groups = []
    ...
    for runner_id in range(n_instances):
        async def _start_container():
            ...
            for _ in range(max_attempts):
                try:
                    ...
                    started = await aci_client.container_groups.begin_create_or_update(
            ...
            heappush(container_groups, (datetime.now(), container_group_name, started))
            aci_client, auth_client = await assign_contributor_permissions(
                aci_client,
                auth_client,
                max_undead,
                credential,
        ...
        tasks.append(_start_container())
    
    await asyncio.gather(*tasks)
    # TODO: Check if need returned values
    await assign_contributor_permissions(
        aci_client,
        auth_client,
        0,
        credential,
                    ...
                    try:
                        ...
                        if iview is not None:
                            ...
                            if state == "Terminated":
                                ...
                                # does not exist
                                deleted = await aci_client.container_groups.begin_delete(
                                    resource_group_name, container_group_name
                                )
