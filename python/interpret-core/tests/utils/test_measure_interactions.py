...
def test_min_hessian(regression_data):
    X, y = regression_data

    min_hessian = 1.1

    ranked_pairs = measure_interactions(X, y, min_hessian=min_hessian)
    assert 6 == len(ranked_pairs)
...
def test_added_impure_contribution_is_zero():
    ...
    ranked_strengths_pure_int = dict(
        measure_interactions(X, y, min_hessian=1e-99, sample_weight=sample_weight)
    )
