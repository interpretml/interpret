...
from .utils import _process_terms, make_all_histogram_edges, _order_terms, _remove_unused_higher_bins, \
    _generate_term_names, _generate_term_types
from ...utils._binning import determine_min_cols, clean_X, clean_dimensions, typify_classification, construct_bins, \
    bin_native_by_dimension, unify_data2, _deduplicate_bins, normalize_initial_seed
from .bin import ebm_decision_function, ebm_decision_function_and_explain, make_boosting_weights, after_boosting, \
    remove_last2, make_bin_weights, trim_tensor, eval_terms
...
class EBMModel(BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None, init_score=None):  # noqa: C901
        ...
        if init_score is not None:
            ...
            if len(y) != len(init_score):
                msg = f"y has {len(y)} samples and sample_weight has {len(init_score)} samples"
                _log.error(msg)
                raise ValueError(msg)
            init_score = init_score.astype(np.float64, copy=False)
        else:
            init_score = np.zeros(len(y)).astype(np.float64, copy=False)
        ...
        if n_classes == 1:
            ...
            dataset = bin_native_by_dimension(
                n_classes,
                1,  # TODO: instead of going back to the original data in X, we
            ...
            if isinstance(interactions, int) and 0 < interactions or not isinstance(interactions, int) and 0 < len(
                    interactions):
                ...
                if isinstance(interactions, int):
                    ...
                    # print(bagged_ranked_interaction)
        ...
        if isinstance(interactions, int) and 0 < interactions:
            return self, bagged_ranked_interaction
        else:
            return self
    ...
    def _to_inner_jsonable(self, properties='interpretable'):
        ...
        for term_idx in range(len(self.term_features_)):
            term = {}
            term['term_features'] = [self.feature_names_in_[feature_idx] for feature_idx in
                                     self.term_features_[term_idx]]
    ...
    def decision_function(self, X, init_score=None):
        ...
        if init_score is not None:
            ...
            init_score = np.zeros(n_samples).astype(np.float64, copy=False)
    ...
    def explain_global(self, name=None):
        ...
        for term_idx, feature_idxs in enumerate(self.term_features_):
            ...
            mod_standard_deviations[term_idx] = trim_tensor(mod_standard_deviations[term_idx],
        ...
        return EBMExplanation(
            ...
            feature_types=['categorical' if x == 'nominal' or x == 'ordinal' else x for x in
                           [term_types[i] for i in keep_idxs]],
            name=name,
            selector=gen_global_selector2(getattr(self, 'n_samples_', None), self.n_features_in_,
                                          [term_names[i] for i in keep_idxs],
                                          ['categorical' if x == 'nominal' or x == 'ordinal' else x for x in
                                           [term_types[i] for i in keep_idxs]],
    ...
    def explain_local(self, X, y=None, name=None):
        ...
        if n_samples == 0:
            ...
            for term_idx, bin_indexes in eval_terms(X, n_samples, self.feature_names_in_, self.feature_types_in_,
                                                    self.bins_, self.term_features_):
