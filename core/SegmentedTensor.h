...
public:
   ...
   bool Expand(const size_t * const acDivisionsPlusOne) {
      ...
      do {
         ...
         EBM_ASSERT(!IsMultiplyError(cValues1, cDivisions1 + 1)); // we check for simple multiplication overflow from m_cStates in EbmTrainingState->Initialize when we unpack featureCombinationIndexes
         ...
         EBM_ASSERT(!IsMultiplyError(cNewValues, cDivisionsPlusOne)); // we check for simple multiplication overflow from m_cStates in EbmTrainingState->Initialize when we unpack featureCombinationIndexes
   ...
   bool Add(const SegmentedTensor & rhs) {
      ...
      do {
         ...
         EBM_ASSERT(!IsMultiplyError(cNewValues, cNewSingleDimensionDivisions + 1)); // we check for simple multiplication overflow from m_cStates in EbmTrainingState->Initialize when we unpack featureCombinationIndexes
